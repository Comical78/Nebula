...
from PyQt6.QtWidgets import (
    QApplication, QMainWindow, QPushButton, QWidget, QVBoxLayout, QLabel, QFrame, QScrollArea
)
from PyQt6.QtCore import QPropertyAnimation, QRect, QTimer, QEasingCurve, Qt, QPoint
...
class AnnouncementOverlay(QWidget):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet("background-color: rgba(0, 0, 0, 0.7);")  # Darken the background
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.box.setGeometry(0, 300, 50, 50)

        # Create a test button
        test_button = QPushButton("Show Announcement", self)
        test_button.clicked.connect(self.show_announcement_dialog)
        layout.addWidget(test_button, alignment=Qt.AlignmentFlag.AlignCenter)

        # Preload the announcement overlay
        print("Loading announcement overlay...")
        self.announcement_overlay = AnnouncementOverlay(self)
        self.announcement_overlay.hide()
        print("Announcement overlay loaded.")

        # Start the box animation
        self.start_box_animation()

    def start_box_animation(self):
        self.animation = QPropertyAnimation(self.box, b"geometry")
        self.animation.setDuration(2000)
        self.animation.setStartValue(QRect(0, 300, 50, 50))
        self.animation.setEndValue(QRect(950, 300, 50, 50))
    ...
    def show_announcement_dialog(self):
        ...
        self.announcement_overlay.show()
        self.animation.resume()  # Resume the animation after the announcement is closed
